...
curl -k -O https://raw.githubusercontent.com/code7a/lab-scripts-dev/main/sample-apps/yelb-with-custom-label-annotations.yml
...
labels_redis_href=$(curl -k -u $auth_username:$session_token https://$PublicDnsName:8443/api/v2/orgs/1/labels -H 'Content-Type: application/json' --data-raw '{"key":"role","value":"R-REDIS"}' | jq -r '.href')
echo $? | grep 0 || labels_redis_href=$(curl -k -s -u $auth_username:$session_token "https://$PublicDnsName:8443/api/v2/orgs/1/labels?key=role&value=R-REDIS" | jq -r .[].href)
#create app role label
labels_app_href=$(curl -k -u $auth_username:$session_token https://$PublicDnsName:8443/api/v2/orgs/1/labels -H 'Content-Type: application/json' --data-raw '{"key":"role","value":"R-APP"}' | jq -r '.href')
echo $? | grep 0 || labels_app_href=$(curl -k -s -u $auth_username:$session_token "https://$PublicDnsName:8443/api/v2/orgs/1/labels?key=role&value=R-APP" | jq -r .[].href)
#create db role label
labels_db_href=$(curl -k -u $auth_username:$session_token https://$PublicDnsName:8443/api/v2/orgs/1/labels -H 'Content-Type: application/json' --data-raw '{"key":"role","value":"R-DB"}' | jq -r '.href')
echo $? | grep 0 || labels_db_href=$(curl -k -s -u $auth_username:$session_token "https://$PublicDnsName:8443/api/v2/orgs/1/labels?key=role&value=R-DB" | jq -r .[].href)
...
labels_yelb_href=$(curl -k -u $auth_username:$session_token https://$PublicDnsName:8443/api/v2/orgs/1/labels -H 'Content-Type: application/json' --data-raw '{"key":"app","value":"A-YELB"}' | jq -r '.href')
echo $? | grep 0 || labels_yelb_href=$(curl -k -s -u $auth_username:$session_token "https://$PublicDnsName:8443/api/v2/orgs/1/labels?key=app&value=A-YELB" | jq -r .[].href)
#get prod label href
